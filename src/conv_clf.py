...
tokenizer.fit_on_texts(data_df['sentence'])
...
X_train, X_test, Y_train, Y_test = train_test_split(data_df['sentence'].values,data_df['label_tec'], test_size = 0.20, random_state = 42, stratify=Y)
...
def create_model(): #66,9% con stop words 

    embedding_layer = Embedding(MAX_NB_WORDS, EMBEDDING_DIM, input_length=X_train_vec.shape[1])
    model = Sequential()
